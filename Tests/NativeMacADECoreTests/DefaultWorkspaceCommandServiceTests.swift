...
struct DefaultWorkspaceCommandServiceTests {
    ...
    @Test
    func loadAppPreferencesAppliesPresentPortableConfigBeforeReturningRuntimePreferences() async throws {
        let now = Date(timeIntervalSince1970: 1_717_394_100)
        let harness = makeHarness(now: { now })
        ...
        #expect(loadedPreferences.themeID == "dracula")
        #expect(loadedPreferences.terminalFontSize == 18)
        #expect(loadedPreferences.focusWorkspaceEnabled)
        #expect(loadedPreferences.defaultSessionShortcutID == codex.id)
        #expect(loadedPreferences.keybindings[.openSettings]?.keyEquivalent == "m")
        #expect(loadedPreferences.updatedAt == now)
        #expect(persistedPreferences == loadedPreferences)
        #expect(harness.store.appPreferences == loadedPreferences)
        #expect(harness.service.metrics.portableSettingsSectionRejectedCount == 0)
        #expect(harness.service.logger.events.contains { event in
            event.name == "portable_settings_loaded" &&
                event.fields["file_present"] == "true" &&
                event.fields["applied_section_count"] == "4" &&
                event.fields["source"] == "startup"
        })
    }

    @Test
    func missingPortableConfigSeedsOnceFromNonDefaultSQLitePreferences() async throws {
        let harness = makeHarness()
        let override = KeybindingOverride(
        ...
        let firstLoad = try await harness.service.loadAppPreferences()
        let seededConfig = try loadedPortableConfig(from: harness.portableSettingsFileStore)
        let secondLoad = try await harness.service.loadAppPreferences()

        #expect(firstLoad.themeID == "catppuccin")
        #expect(firstLoad.terminalFontSize == 17)
        #expect(firstLoad.focusWorkspaceEnabled)
        #expect(seededConfig.appearance?.themeID == "catppuccin")
        #expect(seededConfig.appearance?.terminalFontSize == 17)
        #expect(seededConfig.behavior?.focusWorkspaceEnabled == true)
        #expect(seededConfig.keybindings.map(\.commandID) == [AppCommandID.searchSessions.rawValue])
        #expect(secondLoad.themeID == firstLoad.themeID)
        #expect(harness.service.metrics.portableSettingsSeededCount == 1)
        #expect(harness.service.logger.events.filter { $0.name == "portable_settings_seeded" }.count == 1)
    }

    @Test
    func missingPortableConfigLeavesFileAbsentForPristineDefaults() async throws {
        let harness = makeHarness()

        let loadedPreferences = try await harness.service.loadAppPreferences()

        #expect(loadedPreferences == .defaults)
        #expect(FileManager.default.fileExists(atPath: harness.portableSettingsFileStore.canonicalURL.path) == false)
        #expect(harness.service.metrics.portableSettingsSeededCount == 0)
        #expect(harness.service.logger.events.contains { event in
            event.name == "portable_settings_loaded" &&
                event.fields["file_present"] == "false" &&
                event.fields["seeded_from_sqlite"] == "false"
        })
    }

    @Test
    func saveAppPreferencesExportsPortableSubsetAndSurfacesWriteFailure() async throws {
        let root = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("native-mac-ade-blocked-portable-settings-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let blockedParent = root.appendingPathComponent("blocked-parent")
        try Data([0x00]).write(to: blockedParent)
        let blockedStore = PortableSettingsFileStore(
            canonicalURL: blockedParent.appendingPathComponent("settings.json")
        )
        let harness = makeHarness(portableSettingsFileStore: blockedStore)

        do {
            try await harness.service.saveAppPreferences(AppPreferences(themeID: "dracula"))
            Issue.record("Expected portable settings export failure")
        } catch let error as PortableSettingsFileStoreError {
            guard case .writeFailed = error else {
                Issue.record("Unexpected portable settings error: \(error)")
                return
            }
        }

        #expect(try await harness.persistence.loadAppPreferences() == .defaults)
        #expect(harness.store.appPreferences == .defaults)
        #expect(harness.service.metrics.portableSettingsExportFailureCount == 1)
        #expect(harness.service.metrics.settingsSaveFailureCount == 1)
        #expect(harness.service.logger.events.contains { $0.name == "portable_settings_export_failed" })
    }

    @Test
    func manualPortableReloadUsesSharedApplyPathAndReportsRejectedSections() async throws {
        let now = Date(timeIntervalSince1970: 1_717_394_110)
        let harness = makeHarness(now: { now })
        ...
        let result = try await harness.service.reloadPortableSettingsConfig()
        let persistedPreferences = try await harness.persistence.loadAppPreferences()

        #expect(result.appliedSections == [.appearance])
        ...
        #expect(persistedPreferences.updatedAt == now)
        #expect(harness.service.metrics.portableSettingsReloadCount == 1)
        #expect(harness.service.metrics.portableSettingsPartialApplyCount == 1)
        #expect(harness.service.metrics.portableSettingsSectionRejectedCount == 1)
        #expect(harness.service.logger.events.contains { $0.name == "portable_settings_reloaded" })
        #expect(harness.service.logger.events.contains { event in
            event.name == "portable_settings_section_rejected" &&
                event.fields["section"] == "keybindings" &&
                event.fields["source"] == "manual_reload"
        })
    }
    ...
    func savingFocusWorkspacePreferenceTransitionsRecordsLocalObservability() async throws {
        ...
        try await harness.service.saveAppPreferences(AppPreferences(focusWorkspaceEnabled: true))
        try await harness.service.saveAppPreferences(AppPreferences(focusWorkspaceEnabled: false))

        let enableEvent = try #require(harness.service.logger.events.first { $0.name == "focus_workspace_enabled" })
        let disableEvent = try #require(harness.service.logger.events.first { $0.name == "focus_workspace_disabled" })
        #expect(session.id == harness.store.selectedSessionID)
        #expect(harness.service.metrics.focusWorkspaceEnableCount == 1)
        #expect(harness.service.metrics.focusWorkspaceDisableCount == 1)
        #expect(enableEvent.fields["source"] == "settings_save")
        #expect(enableEvent.fields["selected_session_id_present"] == "true")
        #expect(disableEvent.fields["source"] == "settings_save")
        #expect(disableEvent.fields["selected_session_id_present"] == "true")
        #expect(harness.service.logger.events.filter { $0.name == "focus_workspace_enabled" }.count == 1)
    }
    ...
    @Test
    func focusWorkspaceRejectsTerminalCreationFlowsWithoutMutation() async throws {
        let harness = makeHarness()
        let project = try await harness.service.openProject(path: makeTemporaryProjectDirectory())
        let shortcut = SessionShortcut(
            label: "Focus Agent",
        ...
        let operations: [() async throws -> WorkspaceTab] = [
            { try await harness.service.createTab(sessionID: session.id) },
            { try await harness.service.createPlainTab(sessionID: session.id) },
            { try await harness.service.createDefaultAgentTab(sessionID: session.id) },
            { try await harness.service.createAgentTab(sessionID: session.id, shortcutID: shortcut.id) }
        ]
        ...
        let blockedEvents = harness.service.logger.events.filter { $0.name == "focus_workspace_blocked" }
        #expect(harness.service.metrics.focusWorkspaceBlockedTerminalTabCount == 4)
        #expect(harness.service.metrics.focusWorkspaceBlockedFileTabCount == 0)
        #expect(blockedEvents.count == 4)
        #expect(blockedEvents.allSatisfy { event in
            event.fields["session_id"] == session.id.uuidString &&
                event.fields["reason"] == "additional_terminal_tab_blocked" &&
                event.fields["terminal_tab_count"] == "1" &&
                event.fields["file_tab_count"] == "0" &&
                event.fields["selected_tab_kind"] == "terminal"
        })
    }
    ...
    @Test
    func focusWorkspaceAllowsFirstFileTabAndSameFileReuse() async throws {
        let harness = makeHarness()
        let projectPath = try makeTemporaryProjectDirectory()
        let fileURL = try makeTemporaryProjectFile(in: projectPath, relativePath: "Sources/File.swift")
        let project = try await harness.service.openProject(path: projectPath)
        ...
        let reusedTab = try await harness.service.openFileTab(sessionID: session.id, path: fileURL.path)

        #expect(fileTab.id == reusedTab.id)
        #expect(harness.store.tabsForSelectedSession.map(\.id) == [terminalTab.id, fileTab.id])
        #expect(harness.store.selectedTabID == fileTab.id)
        ...
        #expect(harness.service.metrics.focusWorkspaceBlockedFileTabCount == 0)
        #expect(harness.service.logger.events.contains { event in
            event.name == "file_tab_opened" &&
                event.fields["tab_id"] == fileTab.id.uuidString &&
                event.fields["reused_tab"] == "false"
        })
        #expect(harness.service.logger.events.contains { event in
            event.name == "file_tab_opened" &&
                event.fields["tab_id"] == fileTab.id.uuidString &&
                event.fields["reused_tab"] == "true"
        })
    }

    @Test
    func focusWorkspaceRejectsSecondDifferentFileTabWithoutMutation() async throws {
        let harness = makeHarness()
        let projectPath = try makeTemporaryProjectDirectory()
        ...
        let terminalTab = try #require(harness.store.tabs.first)
        try await harness.service.saveAppPreferences(AppPreferences(focusWorkspaceEnabled: true))
        let firstFileTab = try await harness.service.openFileTab(sessionID: session.id, path: firstFileURL.path)
        harness.service.logger.clear()

        let tabsBefore = harness.store.tabs
        let persistedTabsBefore = try await harness.persistence.loadTabs()
        let snapshotBefore = try await harness.persistence.loadRestoreSnapshot()
        let fileOpenDurationsBefore = harness.service.metrics.fileOpenDurations
        ...
        let event = try #require(harness.service.logger.events.first { $0.name == "focus_workspace_blocked" })
        #expect(harness.store.tabs == tabsBefore)
        #expect(harness.store.tabsForSelectedSession.map(\.id) == [terminalTab.id, firstFileTab.id])
        #expect(harness.store.selectedTabID == firstFileTab.id)
        #expect(try await harness.persistence.loadTabs() == persistedTabsBefore)
        #expect(try await harness.persistence.loadRestoreSnapshot() == snapshotBefore)
        #expect(harness.service.metrics.fileOpenDurations == fileOpenDurationsBefore)
        #expect(harness.service.metrics.focusWorkspaceBlockedFileTabCount == 1)
        #expect(harness.service.metrics.focusWorkspaceBlockedTerminalTabCount == 0)
        #expect(event.fields["session_id"] == session.id.uuidString)
        #expect(event.fields["reason"] == "additional_file_tab_blocked")
        #expect(event.fields["terminal_tab_count"] == "1")
        #expect(event.fields["file_tab_count"] == "1")
        #expect(event.fields["selected_tab_kind"] == "file")
    }
    ...
    @Test
    func reorderProjectsRejectsDuplicateOrMissingIDsWithoutMutatingState() async throws {
        let harness = makeHarness()
        let firstProject = try await harness.service.openProject(path: makeTemporaryProjectDirectory(named: "first"))
        let secondProject = try await harness.service.openProject(path: makeTemporaryProjectDirectory(named: "second"))
        ...
        let persistedProjectsBefore = try await harness.persistence.loadProjects()
        let snapshotBefore = try await harness.persistence.loadRestoreSnapshot()

        await expectInvalidProjectOrder(containing: "Duplicate") {
            try await harness.service.reorderProjects([firstProject.id, firstProject.id, thirdProject.id])
        }
        await expectInvalidProjectOrder(containing: "every visible project") {
            try await harness.service.reorderProjects([thirdProject.id, firstProject.id])
        }

        #expect(projectsBefore.map(\.id) == [firstProject.id, secondProject.id, thirdProject.id])
    ...
    func reorderTabsRejectsUnknownOrOutOfSessionIDsWithoutMutatingState() async throws {
        ...
        let otherSession = try await harness.service.createSession(projectID: project.id, shortcutID: nil)
        let otherSessionTab = try #require(harness.store.tabs.first { $0.sessionID == otherSession.id })
        try await harness.service.selectSession(id: session.id)
        let tabsBefore = harness.store.tabs
        let persistedTabsBefore = try await harness.persistence.loadTabs()
        let snapshotBefore = try await harness.persistence.loadRestoreSnapshot()

        await expectInvalidTabOrder(containing: "every visible tab") {
            try await harness.service.reorderTabs(
                sessionID: session.id,
                orderedVisibleTabIDs: [secondTab.id, otherSessionTab.id]
            )
        }
        await expectInvalidTabOrder(containing: "every visible tab") {
            try await harness.service.reorderTabs(
                sessionID: session.id,
                orderedVisibleTabIDs: [secondTab.id, UUID()]
            )
        }
        await expectInvalidTabOrder(containing: "Duplicate") {
            try await harness.service.reorderTabs(
                sessionID: session.id,
                orderedVisibleTabIDs: [firstTab.id, firstTab.id]
            )
        }
    ...
    @Test
    func reorderProjectsAndTabsPreserveSelectedIdentifiers() async throws {
        let harness = makeHarness()
        let firstProject = try await harness.service.openProject(path: makeTemporaryProjectDirectory(named: "first"))
        let secondProject = try await harness.service.openProject(path: makeTemporaryProjectDirectory(named: "second"))
        let thirdProject = try await harness.service.openProject(path: makeTemporaryProjectDirectory(named: "third"))
        ...
        #expect(harness.store.projects.map(\.id) == [thirdProject.id, secondProject.id, firstProject.id])
        #expect(harness.store.projects.map(\.sortIndex) == [0, 1, 2])
        #expect(harness.store.selectedProjectID == secondProject.id)

        let session = try await harness.service.createSession(projectID: secondProject.id, shortcutID: nil)
        let firstTab = try #require(harness.store.tabs.first { $0.sessionID == session.id })
        let secondTab = try await harness.service.createTab(sessionID: session.id)
        let thirdTab = try await harness.service.createTab(sessionID: session.id)
        try await harness.service.selectTab(id: secondTab.id)

        try await harness.service.reorderTabs(
            sessionID: session.id,
            orderedVisibleTabIDs: [thirdTab.id, firstTab.id, secondTab.id]
        )

        #expect(harness.store.tabsForSelectedSession.map(\.id) == [thirdTab.id, firstTab.id, secondTab.id])
        #expect(harness.store.tabsForSelectedSession.map(\.ordinal) == [0, 1, 2])
        #expect(harness.store.selectedProjectID == secondProject.id)
        #expect(harness.store.selectedSessionID == session.id)
        #expect(harness.store.selectedTabID == secondTab.id)
    }
    ...
    func reorderTabsRegeneratesRestoreSnapshotFromCanonicalOrder() async throws {
        ...
        try await harness.service.reorderTabs(
            sessionID: session.id,
            orderedVisibleTabIDs: [fileTab.id, secondTerminalTab.id, terminalTab.id]
        )

        let snapshot = try #require(try await harness.persistence.loadRestoreSnapshot())
        #expect(snapshot.selectedProjectID == project.id)
        #expect(snapshot.selectedSessionID == session.id)
        #expect(snapshot.selectedTabID == secondTerminalTab.id)
        #expect(snapshot.tabOrder == [fileTab.id, secondTerminalTab.id, terminalTab.id])
        #expect(try await harness.persistence.loadTabs().map(\.id) == [fileTab.id, secondTerminalTab.id, terminalTab.id])
    ...
    func reorderTabsAfterDegradedRestoreKeepsHiddenPersistedTabsInStableTail() async throws {
        ...
        let store = WorkspaceStore(
            projects: [project],
            sessions: [session],
            tabs: [visibleFirst, visibleSecond],
        ...
        try await service.reorderTabs(
            sessionID: session.id,
            orderedVisibleTabIDs: [visibleSecond.id, visibleFirst.id]
        )

        let persistedSessionTabs = try await persistence.loadTabs().filter { $0.sessionID == session.id }
    ...
    private func makeHarness(
        portableSettingsFileStore: PortableSettingsFileStore? = nil,
        now: @escaping @MainActor () -> Date = Date.init
    ) -> CommandServiceHarness<InMemoryWorkspacePersistenceStore> {
        let store = WorkspaceStore()
        let persistence = InMemoryWorkspacePersistenceStore()
        let terminal = FakeTerminalSurfaceManager()
        ...
        let portableSettingsFileStore = portableSettingsFileStore ?? temporaryPortableSettingsFileStore()
        let service = DefaultWorkspaceCommandService(
            store: store,
            persistenceStore: persistence,
    ...
    private func expectInvalidProjectOrder(
        containing expectedMessagePart: String,
        _ operation: () async throws -> Void
    ) async {
        do {
            try await operation()
            Issue.record("Expected invalid project order error")
        } catch WorkspaceCommandError.invalidProjectOrder(let message) {
            #expect(message.contains(expectedMessagePart))
        } catch {
            Issue.record("Unexpected error: \(error)")
        }
    }

    private func expectInvalidTabOrder(
        containing expectedMessagePart: String,
        _ operation: () async throws -> Void
    ) async {
        do {
            try await operation()
            Issue.record("Expected invalid tab order error")
        } catch WorkspaceCommandError.invalidTabOrder(let message) {
            #expect(message.contains(expectedMessagePart))
        } catch {
            Issue.record("Unexpected error: \(error)")
        }
